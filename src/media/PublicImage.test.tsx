...
it("retries an unavailable image without replacing the surrounding content", async () => {
  ...
  render(
    <>
      ...
      <PublicImage attachment={attachment} images={images} />
    </>
  );
...
it("opens a public post image at full size", async () => {
  vi.stubGlobal("URL", {
    createObjectURL: vi.fn().mockReturnValue("blob:portrait"),
    revokeObjectURL: vi.fn()
  });
  const images = { upload: vi.fn(), load: vi.fn().mockResolvedValue(new Blob(["image"])) };
  render(<PublicImage attachment={attachment} images={images} expandable />);
  await userEvent.click(await screen.findByRole("button", { name: "Open image: Portrait" }));
  expect(screen.getByRole("dialog", { name: "Portrait" })).toBeVisible();
});
